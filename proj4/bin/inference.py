    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for  n in newPosDist:
                allPossible[n] =  allPossible[n] +self.beliefs[p] * newPosDist[n] 
        allPossible.normalize()
        self.beliefs = allPossible
    def getBeliefDistribution(self):
        return self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        import math
        self.particleList = []
        particlesRemaining = self.numParticles
        
        legalPos = self.legalPositions
        
        separationSize = int(math.ceil(float(len(legalPos)) / float(self.numParticles)))
        location = 0
        while (particlesRemaining > 0):
            if location >= len(legalPos):
                location = location % len(legalPos)
            self.particleList.append(legalPos[location])
            location += separationSize
            particlesRemaining = particlesRemaining - 1
    ...
    def getBeliefDistribution(self):
        ...
        self.beliefs = util.Counter()
        for p in self.legalPositions: 
                self.beliefs[p] = float(self.particleList.count(p)) / float(self.numParticles)
        self.beliefs[self.getJailPosition()] = float(self.particleList.count(self.getJailPosition())) / float(self.numParticles)
        return self.beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        positionSpace = itertools.permutations(self.legalPositions,self.numGhosts)
        self.legalPos = []
        for value in positionSpace:
            self.legalPos.append(value)
        particlesRemaining = self.numParticles
        separationSize = int(math.ceil(float(len(self.legalPos)) / float(self.numParticles)))
        location = 0
        while (particlesRemaining > 0):
            if location >= len(self.legalPos):
                location = location % len(self.legalPos)
            self.particleList.append(self.legalPos[location])
            location += separationSize
            particlesRemaining = particlesRemaining - 1
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        "*** YOUR CODE HERE ***"
        newParticleList = []
        for pInd in range(len(self.particleList)):
            newParticle = []
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(self.particleList[pInd])), i, self.ghostAgents[i])
    ...
    def getBeliefDistribution(self):
        ...
        """for p in self.legalPos: 
                self.beliefs[p] = float(self.particleList.count(p)) / float(self.numParticles)
        self.beliefs[self.getJailPosition(self.numGhosts)] = float(self.particleList.count(self.getJailPosition(self.numGhosts))) / float(self.numParticles)
        return self.beliefs"""
        
        for part in self.particleList: self.beliefs[part] += 1
        self.beliefs.normalize()
        return self.beliefs
