...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            likelyPos = max(dist, key = dist.get)
            mazeDist[likelyPos] = self.distancer.getDistance(gameState.getPacmanPosition(), likelyPos)
        closestLikelyPos = min(mazeDist, key = mazeDist.get)
        
        distAfterActions = {}
        for action in legal:
            distAfterActions[action]= self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,action), closestLikelyPos)
        return min(distAfterActions, key = distAfterActions.get)
